...
constexpr uint8_t SOCKET_REG(uint8_t socket_n) { return socket_n * 4 + 1; }
...
class Register {
  public:
    explicit Register(uint8_t off, uint8_t sz = 1) : offset(off), size(sz) {}
...
class SocketRegister : public Register {
    using Register::Register;
};
