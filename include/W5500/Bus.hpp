...
class Bus {
  ...
  public:
    ...
    virtual void init() { _lfsr_state = millis(); };
    ...
    virtual uint64_t random() {
        ...
        const uint8_t popcnt_taps_reg =
            __builtin_popcountll(taps & _lfsr_state);
    ...
    void spi_xfer(uint8_t send) { spi_xfer(&send, nullptr, 1); }
    ...
    // this method from it.
    void trigger_interrupt() { _interrupt_pending = true; }
    bool has_pending_interrupt() { return _interrupt_pending; }

  protected:
    void clear_interrupt_flag() { _interrupt_pending = false; }
    friend class W5500;
